#=========================================================================================
# Description: "abs_res" monitor model
#
...
for (i in 0:178){
  ...
  p = 2
  q = 2
  arma_fit = arima(res_train_spline, order = c(p,0,q), include.mean = FALSE, optim.control = list(maxit = 400))
  ...
  log_vib_test = log(vib_test)
  log_vib_test[is.infinite(log_vib_test)] = NA # remove inf !!
  res_test_spline = (log_vib_test - spline_predict)/abs_res_predict_test ## not run !!!
...
save(df_loop_sd, file = "df_loop_sd_1_abs_res.RData") #maxit 1000
